...
class CommunityRecommenderPromptEnhancer:
    ...
    def process_community_report(self, community_report_path):
        """
        Process a community report directly from its file path
        
        Args:
            community_report_path (str): Path to the community report parquet file
            
        Returns:
            torch.Tensor: The community embeddings for recommendation enhancement
        """
        start_total = time.time()
        print(f"\n[DEBUG] Processing community report: {community_report_path}")
        
        try:
            # Check if file exists
            if not os.path.exists(community_report_path):
                print(f"[ERROR] Community report file not found: {community_report_path}")
                return None
                
            # Generate a hash for caching
            import hashlib
            dialogue_hash = hashlib.md5(community_report_path.encode()).hexdigest()
            
            # Check cache first
            cached_embeddings = self.get_cached_embeddings(dialogue_hash)
            if cached_embeddings is not None:
                print(f"[INFO] Using cached embeddings for {community_report_path}")
            ...
            try:
                # Load dialogue communities directly from the report path
                dialogue_communities = pd.read_parquet(community_report_path)
                # Process only essential columns for dialogue communities
                essential_cols = ['community', 'title', 'summary', 'findings']
                cols_to_keep = [col for col in essential_cols if col in dialogue_communities.columns]
                dialogue_communities = dialogue_communities[cols_to_keep].copy()
            ...
            if not similar_communities:
                print("[WARNING] No similar communities found in this dialogue!")
                return None
            ...
            print(f"[ERROR] An error occurred in process_community_report: {e}")
            import traceback
            traceback.print_exc()
            return None
